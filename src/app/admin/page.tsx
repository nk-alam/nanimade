...
export default function AdminDashboard() {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        setStats({
          ...
          conversionRate: data.performanceMetrics[3]?.value || 0
        })
        
        // Set recent orders
        setRecentOrders(data.recentOrders.map((order: any) => ({
          id: order.id.substring(0, 6),
          customerName: 'Customer Name', // We don't have customer names in this data
          customerEmail: 'customer@example.com', // We don't have customer emails in this data
          amount: order.amount,
          status: order.status as 'pending' | 'processing' | 'shipped' | 'delivered' | 'cancelled',
          date: new Date(order.date).toLocaleDateString(),
          items: Math.floor(Math.random() * 5) + 1 // Mock data
        })))
        
        // Set top products
        setTopProducts(data.topProducts.map((product: any) => ({
          id: product.id,
          name: `Product ${product.id.substring(0, 4)}`,
          sales: product.sales,
        ...
        setPerformanceMetrics(data.performanceMetrics.map((metric: any) => ({
          name: metric.name,
          value: metric.value,
  ...
  }, [status, session, selectedTimeRange])

  const getMetricIcon = (name: string) => {
    switch (name) {
      case 'Avg. Session Duration': return <Clock className="h-5 w-5" />
      case 'Bounce Rate': return <TrendingDown className="h-5 w-5" />
      case 'Page Views': return <Eye className="h-5 w-5" />
      case 'Conversion Rate': return <Target className="h-5 w-5" />
  ...
  const getMetricColor = (name: string) => {
    switch (name) {
      case 'Avg. Session Duration': return 'text-blue-600'
      case 'Bounce Rate': return 'text-green-600'
      case 'Page Views': return 'text-purple-600'
      case 'Conversion Rate': return 'text-orange-600'
          ...
          >
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-sm text-green-600">+{stats?.revenueGrowth.toFixed(1)}% from last month</p>
              </div>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-sm text-green-600">+{stats?.orderGrowth.toFixed(1)}% from last month</p>
              </div>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-sm text-green-600">+{stats?.customerGrowth.toFixed(1)}% from last month</p>
              </div>
            </div>
          </motion.div>
