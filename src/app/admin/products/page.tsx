    ...
    const fetchProducts = async () => {
      try {
        ...
        const transformedProducts = data.products.map((product: any) => ({
          ...
          sold: Math.floor(Math.random() * 500), // Mock sold data
          status: product.is_active ? (product.stock_quantity > 0 ? 'active' : 'out_of_stock') : 'draft',
          image: product.images?.[0] || '/api/placeholder/300/300',
          rating: 4.5 + Math.random() * 0.5, // Mock rating
          reviews: Math.floor(Math.random() * 200), // Mock reviews
