...
export async function POST(request: NextRequest) {
  try {
    // Check rate limit
    const rateLimitResult = await withRateLimit(request, 3, 15 * 60 * 1000) // 3 registrations per 15 minutes
    ...
    if (insertError) {
      ...
      return createSecureErrorResponse('Failed to create user', 500)
    }
