...
console.log('Supabase Config in register route:');
console.log('URL:', supabaseUrl);
console.log('Service Key exists:', !!supabaseServiceKey);

if (!supabaseUrl || !supabaseServiceKey) {
  console.error('Supabase environment variables not set in register route')
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: authUser, error: authError } = await supabase.auth.admin.createUser({
      email,
      password, // Using the original password
      email_confirm: false,
    })

    if (authError) {
      console.error('Auth user creation error:', authError)
      return createSecureErrorResponse('Failed to create user account: ' + authError.message, 500)
    }
